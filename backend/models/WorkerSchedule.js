...
const workerScheduleSchema = new mongoose.Schema({
  ...
  // Ngày update lần cuối
  lastUpdated: {
    type: Date,
    default: Date.now
  },
  // Tự động create lịch rãnh cho số ngày tới
  autoScheduleDays: {
    type: Number,
    default: 7 // Tự động create lịch cho 7 ngày tới
  }
}, { 
  timestamps: true 
});
...
// Static method để create lịch mặc định cho thợ
...
// Method để update công việc hiện tại và create lịch rãnh sau thời gian dự kiến
workerScheduleSchema.methods.updateCurrentJob = function(bookingId, estimatedCompletionTime, actualStartTime = new Date()) {
  ...
  // Tự động create lịch rãnh sau thời gian dự kiến hoàn thành
  this.generateSlotsAfterCompletion();
};

// Method để create lịch rãnh sau khi hoàn successful việc hiện tại
workerScheduleSchema.methods.generateSlotsAfterCompletion = function() {
  ...
  if (nextAvailableTime < endOfDay) {
    ...
    while (currentSlotStart < endOfDay) {
      ...
      // Chỉ create slot nếu có ít nhất 1 giờ
      if (currentSlotEnd.getTime() - currentSlotStart.getTime() >= 60 * 60 * 1000) {
        this.availableSlots.push({
          startTime: new Date(currentSlotStart),
          endTime: new Date(currentSlotEnd),
          isBooked: false,
          note: 'Rãnh sau khi hoàn successful việc hiện tại'
        });
      }
  ...
  // Create lịch cho ngày hôm sau nếu hoàn thành muộn
...
// Method để tự động create khung giờ rãnh với các slot mặc định
workerScheduleSchema.methods.generateSlotsForDays = function(days) {
  ...
  for (let i = 0; i < days; i++) {
    ...
    // Check nếu là ngày làm việc
    if (this.workingDays.includes(currentDate.getDay())) {
      // Create các khung giờ mặc định
      const allTimeSlots = [
      ...
      allTimeSlots.forEach(timeSlot => {
        ...
        // Không create slot cho thời gian đã qua
        if (slotStart > now) {
          slots.push({
            startTime: slotStart,
            endTime: slotEnd,
...
// Method để thợ update lịch khả dụng sau khi receive đơn
workerScheduleSchema.methods.updateAvailabilityAfterBooking = function(completedBookingTime, additionalDays = 3) {
  ...
  // Create lịch khả dụng cho các ngày tiếp theo
  this.generateSlotsForDays(additionalDays);
  this.lastUpdated = new Date();
};

// Method để thợ tự update khung giờ khả dụng
workerScheduleSchema.methods.updateCustomAvailability = function(date, availableHours) {
  ...
  // Delete các slot cũ của ngày này (chỉ những slot chưa được đặt)
  this.availableSlots = this.availableSlots.filter(slot => {
  ...
  // Create slot mới theo giờ thợ chọn
  availableHours.forEach(timeSlot => {
    ...
    // Chỉ create slot cho thời gian tương lai
    if (slotStart > new Date()) {
      this.availableSlots.push({
        startTime: slotStart,
        endTime: slotEnd,
        isBooked: false,
        note: `Thợ tự update - ${timeSlot}`
      });
    }
  });
