...
router.post('/estimate', async (req, res) => {
  try {
    ...
    const estimate = await PricingEngine.getEstimate(serviceType, district, urgency);
...
router.post('/book', auth, async (req, res) => {
  try {
    ...
    res.status(201).json({
      ...
      message: autoAssign ? 
        'Đơn hàng đã được create và đang tìm thợ phù hợp' : 
        'Đơn hàng đã được create successful'
    });
    ...
    res.status(500).json({
      success: false,
      message: 'Không thể create đơn hàng'
    });
  }
});
...
router.post('/worker/location', auth, async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: 'Không thể update vị trí'
    });
  }
});
...
router.post('/worker/status', auth, async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: 'Không thể update trạng thái'
    });
  }
});
