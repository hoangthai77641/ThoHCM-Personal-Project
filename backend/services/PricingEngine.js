class PricingEngine {
   ...
   */
  static async calculatePrice(bookingRequest) {
    const {
      serviceDetails,
      location,
      scheduledTime,
    ...
    // 7. VIP discount - use service promoPercent if available
    if (customerLoyalty === 'vip') {
      let vipDiscountPercent = 0.1; // Default 10%
      
      // Try to get promoPercent from service if serviceId provided
      if (serviceId) {
        try {
          const Service = require('../models/Service');
          const service = await Service.findById(serviceId);
          if (service && service.promoPercent && service.promoPercent > 0) {
            vipDiscountPercent = service.promoPercent / 100;
          }
        } catch (error) {
          console.warn('Could not fetch service promo percent:', error.message);
        }
      }
      
      pricing.vipDiscount = Math.round(subtotal * vipDiscountPercent);
      const discountLabel = vipDiscountPercent === 0.1 ? 'Giảm giá VIP (-10%)' : `Giảm giá VIP (-${Math.round(vipDiscountPercent * 100)}%)`;
      pricing.breakdown.push({
        item: discountLabel,
   ...
   */
  static async getEstimate(serviceType, district, urgency = 'normal') {
    const mockRequest = {
    ...
    const pricing = await this.calculatePrice(mockRequest);
