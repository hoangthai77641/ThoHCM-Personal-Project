...
import { Box, Typography, Paper, List, ListItem, ListItemText, Chip, CircularProgress, Button, Alert } from '@mui/material';
...
export default function NotificationCenter() {
  ...
  async function load() {
    ...
    setError(null);
    try {
      const res = await adminHttp.get('/api/notifications/user');
          ...
          <List dense>
            {items.map((n: NotificationItem) => (
              <ListItem key={n._id} divider>
                <ListItemText
                  primary={n.title || n.type || 'Không có tiêu đề'}
                  secondary={n.body}
                />
